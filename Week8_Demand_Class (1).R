...
do.call(rbind, dat_list[1:5]) %>% View()
...
# last year (ie, sub1 and mdat1 where "years_ago" == 1).
...
shares1 <- exp(coefs1) / sum(exp(coefs1)) # <-- formula from bottom of slide for MNL model
round(shares1, 3)

# print the actual brand market shares
# Estimated value (How much of consumer will buy brand) is similar to the real brand shares
brand_shares

# print the actual product market shares - It is NOT YET in the model!
product_shares
...
save(brand_hit_rate, product_hit_rate, ll_ratio, 
     file = "data/mnl_performance_functions.RData")
...
# we did not put many X variables.
...
# Add Price ----- (We are gonna make it better!)
...
# and Samsung are preferred over Huawei. 
# For same price, Apple and Samsung are preferred over Huawei.

#We saw that Apple and Samsung have
...
out4 <- mlogit(choice ~ as.factor(phone_id) + price | 0, data = mdat1)
